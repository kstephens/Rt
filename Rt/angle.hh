...
public:
	angle ( ) {}
	angle ( scalar y, scalar x ) { r = ::atan2(y, x); }

  scalar number() const { return r; }
  scalar radians() const { return r; }
  scalar degrees() const { return 180.0 / M_PI * r; }

	void	asin ( scalar y ) { r = ::asin(y); }
	void	acos ( scalar x ) { r = ::acos(x); }
	void	atan ( scalar y, scalar x ) { r = ::atan2(y, x); }
	void	atan ( scalar yx ) { r = ::atan(yx); }
...
friend	angle	operator op ( const angle& L, scalar s ) { \
		return angle ( L.r op s ); } \
friend	scalar 	operator op ( const angle& L, const angle& R ) { \
		return scalar ( L.r op R.r ); } \
	angle&  operator op##= ( scalar s ) { \
		r op##= s; return *this; }
...
inline scalar	sin (const angle &a )  { return ::sin(a.radians()); }
inline scalar	cos (const angle &a )  { return ::cos(a.radians()); }
inline scalar	tan (const angle &a )  { return ::tan(a.radians()); }
